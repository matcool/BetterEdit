...
#define BE_SETTING_FUNC(__name__) \
	BetterEdit::get##__name__##AsString(), &BetterEdit::set##__name__##FromString

#define BE_SETTING_FUNC_B(__name__) BetterEdit::get##__name__(), &BetterEdit::set##__name__
...
#define USER_LINK(name, gdid)                                     \
	addButton(                                                    \
		CCNodeConstructor<CCLabelBMFont*>()                       \
			...
			.done(),                                              \
		(SEL_MenuHandler) & BESettingsLayer::onShowAccount, false \
	)                                                             \
...
void BESettingsLayer::setup() {
	...
	this->m_pNextPageBtn = CCMenuItemSpriteExtra::create(
		CCNodeConstructor().fromFrameName("GJ_arrow_01_001.png").flipX().done(), this,
		(SEL_MenuHandler)&BESettingsLayer::onPage
	);
	...
	this->addToggle(
		"Custom Grid Size", "Enable Custom Grid Size <cy>(Static)</c>",
		BE_SETTING_FUNC_B(GridSizeEnabled)
	);
	this->addToggle(
		"Always Use Custom Grid Size",
		"Without this option, using the <cr>grid zoom buttons</c> will reset grid behaviour to "
		"normal "
	...
	this->addToggle(
		...
		"When using <cr>Control</c> + <cp>Scroll Wheel</c> to zoom, the screen won't move toward "
		"the cursor",
		BE_SETTING_FUNC_B(DisableMouseZoomMove)
	);
	this->addToggle(
		"Fade Out Percentage",
		"When enabled, the <cl>percentage</c> text below the progress bar will automatically fade "
		"out when "
	...
	this->addToggle(
		"Disable Extra Object Info", nullptr, BE_SETTING_FUNC_B(DisableExtraObjectInfo)
	);
	this->addToggle(
		"Bypass Object Limit", "Disables the 80k Object Limit and 100 Custom Object Limit",
	...
	this->addToggle(
		...
		BE_SETTING_FUNC_B(DisableVisibilityTab), false, true
	);
	this->addToggle(
		"Disable Double Click", "Double-clicking an object opens the <cy>Edit Object</c> menu",
	...
	this->addToggle(
		...
		BE_SETTING_FUNC_B(EnableCustomEditMenu), false, true
	);
	this->addToggle(
		"Become Unfunny", "Disables <cg>Easter Eggs</c>", BE_SETTING_FUNC_B(NoEasterEggs)
	);
	...
	this->addToggle(
		...
		BE_SETTING_FUNC_B(EnableAutoSave), true
	);

	// TODO: add these back
	// this->addSlider(
	// 	"Music", (SEL_MenuHandler)&PauseLayer::musicSliderChanged,
	// 	FMODAudioEngine::sharedEngine()->getMusicVolume()
	// );
	// this->addSlider(
	// 	"SFX", (SEL_MenuHandler)&PauseLayer::sfxSliderChanged,
	...
	this->addButton(
			CCNodeConstructor<CCLabelBMFont*>()
				...
				.scale(.6f)
				.done(),
			(SEL_MenuHandler)&BESettingsLayer::onShowAccount, true
	)
		->setUserData(as<void*>(104257));
	this->addTitle("Ported by:", "bigFont.fnt")->setScale(.6f);
	this->USER_LINK("Mat", 5568872);
	this->USER_LINK("Alk1m123", 11535118);
	...
	this->addTitle("Special Thanks (Part 3):", "bigFont.fnt")->setScale(.6f);
	...
	this->addButton(
			CCNodeConstructor<CCLabelBMFont*>()
				...
				.limit(this->m_pLrSize.width / 2 - 64.0f, .75f, .1f)
				.done(),
			(SEL_MenuHandler)&BESettingsLayer::onFool, false
	)
	...
	this->addButton(
			CCNodeConstructor<ButtonSprite*>()
				...
				.scale(.75f)
				.done(),
			(SEL_MenuHandler)&BESettingsLayer::onURL, true
	)
		->setUserObject(CCString::create("https://discord.gg/K9Kuh3hzTC"));
	this->addTitle(
			(std::string("Running version ") + geode::Mod::get()->getVersion().toString()).c_str(),
			"goldFont.fnt"
	)
		->setScale(.6f);
	this->addToggle(
	...
	this->addTitle(
		"For Feedback / Support, Contact HJfod#1795\nor Join the Discord", "bigFont.fnt"
	);
	// this->addToggle(
	...
	// );
	// this->addToggle(
	//     "Beta Test", "If auto-updates are enabled, automatically updates to the latest version,
	//     even if it is a beta", BE_SETTING_FUNC_B(AutoUpdateEnabled)
	// );
}
...
cocos2d::CCPoint BESettingsLayer::getItemPos(bool large, bool center) {
	if (large) {
		auto y =
			this->m_pLrSize.height / 2 - 64.0f - ((this->m_nItemCount + 1) / 2) * s_fItemPaddingV;
...
void BESettingsLayer::addInput(
	char const* text, std::string const& value, BE_Callback cb, std::string const& filter
) {
	...
	scaleSnapLabel->setPosition(
		winSize.width / 2 - scaleSnapLabel->getScaledContentSize().width / 2, winSize.height / 2 + y
	);
...
void BESettingsLayer::addSlider(char const* text, cocos2d::SEL_MenuHandler onChange, float val) {
	auto winSize = CCDirector::sharedDirector()->getWinSize();
...
CCLabelBMFont* BESettingsLayer::addTitle(char const* text, char const* font) {
	auto winSize = CCDirector::sharedDirector()->getWinSize();
	auto y = getItemPos(true).y;
...
CCLabelBMFont* BESettingsLayer::addSubtitle(char const* text, bool center) {
	auto winSize = CCDirector::sharedDirector()->getWinSize();
	...
	subtitleLabel->setPosition(
		winSize.width / 2 + x +
			(center ? this->m_pLrSize.width / 4 : subtitleLabel->getScaledContentSize().width / 2),
		winSize.height / 2 + y
	);
...
CCMenuItemSpriteExtra* BESettingsLayer::addButton(
	CCNode* sprite, SEL_MenuHandler callback, bool large
) {
...
CCMenuItemSpriteExtra* BESettingsLayer::addButton(
	char const* sprite, SEL_MenuHandler callback, bool large
) {
...
void BESettingsLayer::addToggle(
	char const* text, char const* desc, bool value, BE_Callback_B cb, bool experimental,
	bool showDescOnToggle
) {
	auto toggle = CCMenuItemToggler::createWithStandardSprites(
		this, (SEL_MenuHandler)&BESettingsLayer::onToggle, .75f
	);
	...
	if (desc && strlen(desc)) {
		auto infoButton = CCMenuItemSpriteExtra::create(
			CCNodeConstructor().fromFrameName("GJ_infoIcon_001.png").scale(.5f).done(), this,
	...
	if (experimental) {
		auto eLabel = CCMenuItemSpriteExtra::create(
			CCNodeConstructor<CCLabelBMFont*>()
				...
				.scale(.7f)
				.done(),
			this, (SEL_MenuHandler)&BESettingsLayer::onInfo
		);

		eLabel->setPosition(x + 20.0f + label->getScaledContentSize().width, y + 2.0f);
...
void BESettingsLayer::onInfo(CCObject* pSender) {
	FLAlertLayer::create(
		nullptr, "Info", as<CCString*>(as<CCNode*>(pSender)->getUserObject())->getCString(), "OK",
		nullptr, 380.0f
	)
		->show();
}
...
void BESettingsLayer::onPage(CCObject* pSender) {
	...
	auto add = as<intptr_t>(as<CCNode*>(pSender)->getUserData());
...
void BESettingsLayer::onShowAccount(CCObject* pSender) {
	if (as<CCNode*>(pSender)->getUserData()) {
		ProfilePage::create(as<intptr_t>(as<CCNode*>(pSender)->getUserData()), true)->show();
	}
}
...
void BESettingsLayer::onClose(CCObject* pSender) {
	GameManager::sharedState()->getEditorLayer()->m_editorUI->updateGridNodeSize();
...
void BESettingsLayer::keyDown(enumKeyCodes key) {
	switch (key) {
		case KEY_Left: onPage(m_pPrevPageBtn); break;
		case KEY_Right: onPage(m_pNextPageBtn); break;
		default: BrownAlertDelegate::keyDown(key);
	}
}
...
BESettingsLayer* BESettingsLayer::create(EditorPauseLayer* pause) {
	...
	if (ret && (ret->m_pPauseLayer = pause) &&
		ret->init(340.0f, 240.0f, "GJ_square01.png", "BetterEdit Settings")) {
		ret->autorelease();
		return ret;
	}
