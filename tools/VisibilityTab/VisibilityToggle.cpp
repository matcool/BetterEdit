...
VisibilityToggle* VisibilityToggle::create(
	char const* sprite, VisibilityToggle::Getter getter, VisibilityToggle::Setter setter
) {
	auto ret = new VisibilityToggle;

	if (ret &&
		ret->init(
			createEditBtnSprite(sprite, false), createEditBtnSprite(sprite, true), ret,
...
VisibilityToggle* VisibilityToggle::create(char const* sprite) {
	auto ret = new VisibilityToggle;

	if (ret &&
		ret->init(
			createEditBtnSprite(sprite, false), createEditBtnSprite(sprite, true), ret, nullptr
		)) {
		ret->autorelease();
		return ret;
	}
