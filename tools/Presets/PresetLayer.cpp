...
void PresetLayer::setup() {
	...
	this->m_buttonMenu->addChild(
		CCNodeConstructor<CCMenuItemSpriteExtra*>()
			.fromNode(CCMenuItemSpriteExtra::create(
				CCNodeConstructor().fromFrameName("GJ_arrow_01_001.png").scale(.8f).done(), this,
	...
	this->m_buttonMenu->addChild(
		CCNodeConstructor<CCMenuItemSpriteExtra*>()
			.fromNode(CCMenuItemSpriteExtra::create(
				CCNodeConstructor().fromFrameName("GJ_arrow_01_001.png").scale(.8f).flipX().done(),
				this, (SEL_MenuHandler)&PresetLayer::onPage
			))
			.udata(1)
			.move(this->m_pLrSize.width / 2 - 25.0f, 0.0f)
			.done()
	...
	this->m_buttonMenu->addChild(
		CCNodeConstructor<CCMenuItemSpriteExtra*>()
			.fromNode(CCMenuItemSpriteExtra::create(
				CCSprite::createWithSpriteFrameName("GJ_deleteIcon_001.png"), this,
	...
	this->m_buttonMenu->addChild(
		CCNodeConstructor<CCMenuItemSpriteExtra*>()
			.fromNode(CCMenuItemSpriteExtra::create(
				CCNodeConstructor().fromFrameName("GJ_optionsBtn_001.png").scale(.7f).done(), this,
	...
	this->m_buttonMenu->addChild(
		CCNodeConstructor<CCMenuItemSpriteExtra*>()
			.fromNode(CCMenuItemSpriteExtra::create(
				ButtonSprite::create("Create", 0, 0, "goldFont.fnt", "GJ_button_01.png", 0, .8f),
				this, (SEL_MenuHandler)&PresetLayer::onCreate
			))
...
void PresetLayer::updateLabel(char const* text) {
	this->m_pSelectedLabel->setString(text);
	this->m_pSelectedLabel->limitLabelWidth(this->m_pLrSize.width - 90.0f, 1.0f, .2f);
}

void PresetLayer::onPage(CCObject* pSender) {
	this->m_nSelectedIndex += as<intptr_t>(as<CCNode*>(pSender)->getUserData());
...
void PresetLayer::onCreate(CCObject* pSender) {
	...
	auto level = GameLevelManager::get()->createNewLevel();
	if (lvlStr.size()) {
		level->m_levelString = lvlStr.c_str();
	}
...
void PresetLayer::onRemove(CCObject* pSender) {
	if (this->m_nSelectedIndex) {
		BetterEdit::sharedState()->removePreset(this->m_nSelectedIndex - 1);
	}
...
void PresetLayer::onRename(CCObject* pSender) {
	if (this->m_nSelectedIndex) {
		InputPrompt::create("Rename Preset", "Name", [this](char const* val) -> void {
			if (!this->m_nSelectedIndex) {
				return;
			}
			...
			this->updateLabel(
				BetterEdit::sharedState()->getPresets()[this->m_nSelectedIndex - 1].name.c_str()
			);
		})->show();
	}
}
