...
void IDRemapPopup::setup() {
		...
		CCNodeConstructor<CCMenuItemSpriteExtra*>()
			.fromNode(CCMenuItemSpriteExtra::create(
				CCNodeConstructor<ButtonSprite*>()
					...
					.scale(.8f)
					.done(),
				this, (SEL_MenuHandler)&IDRemapPopup::onRemap
			))
	...
	MAKE_INFOBUTTON(
		...
		-m_pLrSize.width / 2 + 25.0f, -m_pLrSize.height / 2 + 25.0f,
...
void IDRemapPopup::onRemap(CCObject* pSender) {
	...
	if (!(this->m_pStartInput && this->m_pStartInput->getString() &&
		  strlen(this->m_pStartInput->getString()))) {
		return;
	}

	if (!(this->m_pEndInput && this->m_pEndInput->getString() &&
		  strlen(this->m_pEndInput->getString()))) {
		return;
	}

	if (!(this->m_pOffsetInput && this->m_pOffsetInput->getString() &&
	...
	CCARRAY_FOREACH_B_TYPE(LevelEditorLayer::get()->getAllObjects(), obj, GameObject) {
		...
		for (auto i = 0; i < obj->m_groupCount; i++) {
			RANGE_CHECK(obj->m_groups->at(i));
		}

		if (asEffectGameObject(obj)) {
			auto eobj = asEffectGameObject(obj);

			RANGE_CHECK(eobj->m_targetGroupID);
			RANGE_CHECK(eobj->m_centerGroupID);
	...
	FLAlertLayer::create(
		nullptr, "Remap Done",
		"<co>" + std::to_string(editCount) +
			"</c> objects edited\n"
			"<cc>" +
			std::to_string(remapCount) + "</c> groups remapped",
		"OK", nullptr
	)
		->show();
}
